...
fn test_validate_cert<F>(log: F)
		where F: Fn(&'static str) -> () {
	debug::RuntimeLogger::init();

	runtime_print!("test_validate_cert: entered!");
	log("test_validate_cert: entered!");
	...
	log("test_validate_cert: sig decoded!");
	...
	log("test_validate_cert: cert decoded!");
	...
	log("test_validate_cert: verify_is_valid_tls_server_cert!");
	...
	log("test_validate_cert: verify_signature!");
}
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn runtest(origin) -> dispatch::DispatchResult {
			...
			test_validate_cert(|msg: &'static str| {
				Self::deposit_event(RawEvent::LogString(
					msg.as_bytes().to_vec()
				));
			});
			Ok(())
